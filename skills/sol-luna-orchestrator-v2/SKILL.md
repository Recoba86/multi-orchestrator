...
When master routing is **ENABLED** (`orchestrator-routing on` / `use SolMode`):
1. **Boss Binding:** Submitted Boss requests follow the mode-aware Boss priority chain (`SOL_HIGH` -> `GROK_4_6_HIGH` -> `OPUS_4_6_THINKING` -> `GEMINI_FLASH_HIGH`).
...
3. **Reviewer Independence:** `VERIFIER` assignments enforce bidirectional independence (`gpt_family` implementers cannot be reviewed by `gpt_family` candidates).
...
- Exact legacy submitted-request authority is restored: Boss defaults to `SOL_HIGH` (`gpt-5.6-sol`, High effort) and legacy role chains apply.
