...
def render_human_readable(trace_data):
    ...
    for idx, act in enumerate(actions):
        ...
        binding = act.get("binding", {})
        requested_model = binding.get("requested_model", req.get("model", "UNPROVEN"))
        requested_effort = binding.get("requested_effort", req.get("effort", "UNPROVEN"))
        effective_model = binding.get("effective_model", exe.get("effective_model", exe.get("actual_model", "UNPROVEN")))
        effective_effort = binding.get("effective_effort", exe.get("effective_effort", exe.get("actual_effort", "UNPROVEN")))
        fork_mode = exe.get("fork_turns", "UNPROVEN")
        binding_status = binding.get("status")
        if binding_status is None:
            binding_status = {True: "MATCH", False: "MISMATCH"}.get(act.get("binding_match"), "UNPROVEN")
        lines.append(f"      Binding Request:   {requested_model}/{requested_effort}")
        lines.append(f"      Executed Binding: {effective_model} (type: {exe.get('agent_type')}, effort: {effective_effort}, fork_turns: {fork_mode})")
