...
Customized legacy agent declarations (`.codex/agents/*.toml`) are preserved byte-for-byte as unmanaged files and recorded in `migration_omissions`; they are not ownership-shielded from comprehensive syntax, policy, or Core verification. A modified Core, skill, command, model-policy module, or other indispensable payload blocks migration before mutation. Unknown, escaping, symlinked, reserved, hash-conflicting, or ambiguous paths also fail closed.
